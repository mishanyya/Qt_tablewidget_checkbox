...
#include "QTextBrowser"//добавлено для создания элемента, в котором динамически будут созданы другие элементы с прокруткой(пробуем разные!!!)
...
if (checkbox->isChecked())
{
check=ui->tableWidget->item(i,2)->text();//получаем значение в ячейке колонки №2 в таблице
a[i]=check;

ui->textBrowser->append(a[i]);//добавить текст в поле textBrowser
